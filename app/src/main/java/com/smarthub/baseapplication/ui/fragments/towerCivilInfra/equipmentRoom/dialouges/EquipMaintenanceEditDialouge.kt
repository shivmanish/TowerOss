...
class EquipMaintenanceEditDialouge (var data: PreventiveMaintenance, var childId: Int?, var parentId: Int?, var listner:EquipMaintenanceUpdateListener) : BaseBottomSheetDialogFragment(){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.update.setOnClickListener {
            ...
            data.let {
                ...
                tempEquipRoomAllData.PreventiveMaintenance= arrayListOf(it)
            }
            if (childId!=null)
        ...
        viewmodel.updateTwrCivilInfraDataResponse?.observe(viewLifecycleOwner) {
            if (it != null && it.status == Resource.Status.LOADING) {
                showProgressLayout()
                AppLogger.log("EquipMaintenanceEditDialouge data Updating in progress ")
                return@observe
            }
            if (it?.data != null && it.status == Resource.Status.SUCCESS) {
                AppLogger.log("EquipMaintenanceEditDialouge card Data Updated successfully")
                listner.updatedData()
                hideProgressLayout()
                dismiss()
                ...
                AppLogger.log("EquipMaintenanceEditDialouge Something went wrong in updating data")
            }
            else if (it != null) {
                AppLogger.log("EquipMaintenanceEditDialouge error :${it.message}, data : ${it.data}")
            } else {
                AppLogger.log("EquipMaintenanceEditDialouge Something went wrong in updating data")
    ...
    interface EquipMaintenanceUpdateListener{
        fun updatedData()
    }
