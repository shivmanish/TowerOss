...
class PolePoEditAdapter (var data: TwrCivilPODetail, var childId:Int?, var parentId:Int?, var listner:PolePoUpdateListener) : BaseBottomSheetDialogFragment(){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.update.setOnClickListener {
            ...
            data.let {
                ...
                tempPoleAllData.PODetail= arrayListOf(it)
            }
            if (childId!=null)
        ...
        viewmodel.updateTwrCivilInfraDataResponse?.observe(viewLifecycleOwner) {
            if (it != null && it.status == Resource.Status.LOADING) {
                showProgressLayout()
                AppLogger.log("PolePoEditAdapter data Updating in progress ")
                return@observe
            }
            if (it?.data != null && it.status == Resource.Status.SUCCESS) {
                AppLogger.log("PolePoEditAdapter card Data Updated successfully")
                listner.updatedData()
                hideProgressLayout()
                dismiss()
                ...
                AppLogger.log("PolePoEditAdapter Something went wrong in updating data")
            }
            else if (it != null) {
                AppLogger.log("PolePoEditAdapter error :${it.message}, data : ${it.data}")
            } else {
                AppLogger.log("PolePoEditAdapter Something went wrong in updating data")
    ...
    interface PolePoUpdateListener{
        fun updatedData()
    }
}