...
class EarthingMaintenanceEditDialouge (var data: PreventiveMaintenance, var childId: Int?, var parentId: Int?, var listner:EarthMaintenanceUpdateListener) : BaseBottomSheetDialogFragment(){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.update.setOnClickListener {
            ...
            data.let {
                ...
                tempEarthingAllData.PreventiveMaintenance= arrayListOf(it)
            }
            if (childId!=null)
        ...
        viewmodel.updateTwrCivilInfraDataResponse?.observe(viewLifecycleOwner) {
            if (it != null && it.status == Resource.Status.LOADING) {
                showProgressLayout()
                AppLogger.log("EarthingMaintenanceEditDialouge data Updating in progress ")
                return@observe
            }
            if (it?.data != null && it.status == Resource.Status.SUCCESS) {
                AppLogger.log("EarthingMaintenanceEditDialouge card Data Updated successfully")
                listner.updatedData()
                hideProgressLayout()
                dismiss()
                ...
                AppLogger.log("EarthingMaintenanceEditDialouge Something went wrong in updating data")
            }
            else if (it != null) {
                AppLogger.log("EarthingMaintenanceEditDialouge error :${it.message}, data : ${it.data}")
            } else {
                AppLogger.log("EarthingMaintenanceEditDialouge Something went wrong in updating data")
    ...
    interface EarthMaintenanceUpdateListener{
        fun updatedData()
    }
