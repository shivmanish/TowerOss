...
class SiteAcqsitionFragAdapter(var context:Context,var listner:SiteAcqListListener): RecyclerView.Adapter<SiteAcqsitionFragAdapter.ViewHold>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHold {
        return when (viewType) {
            1 ->{
                val view = LayoutInflater.from(parent.context).inflate(R.layout.site_lease_list_item,parent,false)
                DataViewHold(view)
            }
            2 ->{
    ...
    override fun onBindViewHolder(holder: ViewHold, position: Int) {
        when(holder){
            is DataViewHold->{
                ...
                holder.binding.titel.text="Acquisition #${position.plus(1)}"
