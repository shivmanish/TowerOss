...
import com.smarthub.baseapplication.databinding.MwUbrAntennaDetailsBinding
...
import com.smarthub.baseapplication.databinding.MwUbrOduDetailsBinding
...
class MwUbrAntennaFragAdapter(var listener: MwUbrAntennaClickListener, var context: Context, LinkInfoData:ArrayList<BackBackhaulLinkMWAntenna>?) : RecyclerView.Adapter<MwUbrAntennaFragAdapter.ViewHold>() {

    var list = ArrayList<Any>()

    fun setData(data: ArrayList<BackBackhaulLinkMWAntenna>?) {
        this.list.clear()
...
//    }
    ...
    fun updateItem(pos : Int,data : BackBackhaulLinkMWAntenna){
        list[pos] = data
    ...
    class ViewHold1(itemView: View) : ViewHold(itemView) {
        var binding : MwUbrAntennaDetailsBinding = MwUbrAntennaDetailsBinding.bind(itemView)
        init {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHold {
        return when (viewType) {
            1 ->{
                val view = LayoutInflater.from(parent.context).inflate(R.layout.mw_ubr_antenna_details,parent,false)
                ViewHold1(view)
            }
            2 ->{
    ...
    override fun onBindViewHolder(holder: ViewHold, position: Int) {
        if (holder is ViewHold1) {
            val data: BackBackhaulLinkMWAntenna=list[position] as BackBackhaulLinkMWAntenna
...
//                listener.editModeCliked(data,position)
            ...
            try {
                ...
                holder.binding.AntennaMake.text=data.Make
                holder.binding.AntennaModel.text=data.Model
                ...
                holder.binding.HeightAGL.text=data.Height
                holder.binding.Orientation.text=data.Orientation
                holder.binding.Shape.text=data.Shape.toString()
                holder.binding.Tilt.text=data.Tilt
    ...
    override fun getItemViewType(position: Int): Int {
        return if (list.isEmpty())
            2
        else if(list[position] is BackBackhaulLinkMWAntenna)
            1
        else
            3
    }
    ...
    interface MwUbrAntennaClickListener{
        fun editModeCliked(data :BackBackhaulLinkMWAntenna,pos:Int)
    }
}