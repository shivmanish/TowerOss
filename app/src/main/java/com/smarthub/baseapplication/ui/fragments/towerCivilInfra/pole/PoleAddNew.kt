...
class PoleAddNew (var fulldata: NewTowerCivilAllData?, var listner: AddNewPoleListner): BaseBottomSheetDialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewmodel.updateTwrCivilInfraDataResponse?.observe(viewLifecycleOwner) {
            if (it != null && it.status == Resource.Status.LOADING) {
                showProgressLayout()
                AppLogger.log("PoleAddNew Fragment data adding in progress ")
                return@observe
            }
            if (it?.data != null && it.status == Resource.Status.SUCCESS) {
                ...
                AppLogger.log("PoleAddNew Fragment Something went wrong")
            }
            else if (it != null) {
                AppLogger.log("PoleAddNew Fragment error :${it.message}, data : ${it.data}")
            } else {
                AppLogger.log("PoleAddNew Fragment Something went wrong")
        ...
        hideProgressLayout()
        binding.titleText.text="Add Pole"
        binding.titelInfo.text="Pole Details"
    }
