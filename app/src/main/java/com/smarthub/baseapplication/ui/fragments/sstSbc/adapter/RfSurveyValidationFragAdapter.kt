...
class RfSurveyValidationFragAdapter(var baseFragment:BaseFragment, var listener: RfSurveyApprovalListListener, data: SstSbcAllData?) : RecyclerView.Adapter<RfSurveyValidationFragAdapter.ViewHold>() {
    private var datalist: SstSbcObservation?=null


    fun setData(data: SstSbcObservation?) {
        this.datalist=data!!
        notifyDataSetChanged()
    }
    init {
        try {
            if (data!=null && data.SstSbcObservation?.isNotEmpty()==true){
                datalist= data.SstSbcObservation?.get(0)
            }
        }catch (e:java.lang.Exception){
            AppLogger.log("RfSurveyValidationFragAdapter error :${e.localizedMessage}")
        }
    }
    ...
    class ViewHold2(itemView: View, listener: RfSurveyApprovalListListener) : ViewHold(itemView) {
...
//            }
        ...
        when (holder) {
            is ViewHold1 -> {
                ...
                holder.binding.update.setOnClickListener {
                    val tempSurveyApprovalData=SstSbcObservation()
                    ...
                    listener.updateRfSurveyApprovalClicked(tempSurveyApprovalData)
    ...
    interface RfSurveyApprovalListListener {
       fun attachmentItemClicked(item : Attachments)
       fun addAttachment()
       fun updateRfSurveyApprovalClicked(data:SstSbcObservation)
    }
