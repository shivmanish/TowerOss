...
class AddNewFireExtDialouge (var fullData:ArrayList<UtilityEquipmentAllData>?, var listner:AddFireExtDataListener) : BaseBottomSheetDialogFragment(){
...
//
        ...
        viewmodel.updateUtilityDataResponse?.observe(viewLifecycleOwner) {
            if (it != null && it.status == Resource.Status.LOADING) {
                showProgressLayout()
                AppLogger.log("AddNewFireExtDialouge data Updating in progress ")
                return@observe
            }
            if (it?.data != null && it.status == Resource.Status.SUCCESS && it.data.status.UtilityEquipment==200) {
                ...
                AppLogger.log("AddNewFireExtDialouge Something went wrong in updating data")
            }
            else if (it != null) {
                AppLogger.log("AddNewFireExtDialouge Fragment error :${it.message}, data : ${it.data}")
            } else {
                AppLogger.log("AddNewFireExtDialouge  Something went wrong in updating data")
    ...
    interface AddFireExtDataListener{
        fun addNewData()
    }
