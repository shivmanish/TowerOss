...
class MwUbrODUFragAdapter(var listener: MwUbrODUClickListener, var context: Context, LinkInfoData:ArrayList<BackhaulLinkMWODU>?) : RecyclerView.Adapter<MwUbrODUFragAdapter.ViewHold>() {

    var list = ArrayList<Any>()

    fun setData(data: ArrayList<BackhaulLinkMWODU>?) {
        this.list.clear()
...
//    }
    ...
    fun updateItem(pos : Int,data : BackhaulLinkMWODU){
        list[pos] = data
    ...
    class ViewHold1(itemView: View) : ViewHold(itemView) {
        var binding : MwUbrOduDetailsBinding = MwUbrOduDetailsBinding.bind(itemView)
        init {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHold {
        return when (viewType) {
            1 ->{
                val view = LayoutInflater.from(parent.context).inflate(R.layout.mw_ubr_odu_details,parent,false)
                ViewHold1(view)
            }
            2 ->{
    ...
    override fun onBindViewHolder(holder: ViewHold, position: Int) {
        if (holder is ViewHold1) {
            val data: BackhaulLinkMWODU=list[position] as BackhaulLinkMWODU
...
//                listener.editModeCliked(data,position)
    ...
    override fun getItemViewType(position: Int): Int {
        return if (list.isEmpty())
            2
        else if(list[position] is BackhaulLinkMWODU)
            1
        else
            3
    }
    ...
    interface MwUbrODUClickListener{
        fun editModeCliked(data :BackhaulLinkMWODU,pos:Int)
    }
}