...
class BackhaulFiberBottomSheet(
    contentLayoutId: Int,
    var Id: String?,
    ...
    lateinit var binding : BackhaulFiberBottomSheetDialogBinding
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        basicinfoModel = BasicinfoModel()
        binding = BackhaulFiberBottomSheetDialogBinding.bind(view)
        binding.icMenuClose.setOnClickListener {
            dismiss()
        }
        BackhaulFeasibility!!.Fiber!!.get(0).let {
...
//                binding.trxcounter.setText(it.TRXCount)
                binding.opcoforendsite.setText(it.OpcoFarEndSite)
                binding.lmlength.setText(it.LmLength)
                binding.cableLenth.setText(it.CableLength)
            }
        }
        binding.include.update.setOnClickListener {
            BackhaulFeasibility!!.Fiber!!.get(0).let {
//                it.TRXCount = binding.trxcounter.text.toString()
                it.OpcoFarEndSite = binding.opcoforendsite.text.toString()
                it.LmLength = binding.lmlength.text.toString()
                it.CableLength = binding.cableLenth.text.toString()
            }
    ...
    override fun onCreateView(
        ...
        binding = BackhaulFiberBottomSheetDialogBinding.inflate(inflater)
        return binding.root
    }
