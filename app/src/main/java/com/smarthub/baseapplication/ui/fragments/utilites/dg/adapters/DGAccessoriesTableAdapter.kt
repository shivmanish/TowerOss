...
class DGAccessoriesTableAdapter (var context : Context, var listener : DGFragAdapter.DGListListener, var list:ArrayList<UtilityAccessory>?): RecyclerView.Adapter<DGAccessoriesTableAdapter.ViewHold>() {


    fun addItem(){
        val data=UtilityAccessory()
        list!!.add(data)
    ...
    override fun onBindViewHolder(holder: ViewHold, position: Int) {
        val item: UtilityAccessory =list!![position]
    ...
    private fun performOptionsMenuClick(position: Int,view : View,data:UtilityAccessory) {
        ...
        popupMenu.setOnMenuItemClickListener(object : PopupMenu.OnMenuItemClickListener{
            override fun onMenuItemClick(item: MenuItem?): Boolean {
                when(item?.itemId){
                    R.id.action_edit -> {
                        popupMenu.dismiss()
                        listener.editAccessoriesClicked(position,data)
                    ...
                    R.id.action_view -> {
                        popupMenu.dismiss()
                        listener.viewAccessoriesClicked(position,data)
                    }
