...
class SurveyValidationFragAdapter(var baseFragment:BaseFragment, var listener: SurveyApprovalListListener, data:NewSiteAcquiAllData?) : RecyclerView.Adapter<SurveyValidationFragAdapter.ViewHold>() {
    private var datalist: SAcqSiteApproval?=null


    fun setData(data: SAcqSiteApproval?) {
        this.datalist=data!!
        notifyDataSetChanged()
    }
    init {
        try {
            if (data!=null && data.SAcqSiteApproval?.isNotEmpty()==true){
                datalist= data.SAcqSiteApproval?.get(0)
            }
        }catch (e:java.lang.Exception){
            AppLogger.log("SurveyValidationFragAdapter error :${e.localizedMessage}")
        }
    }
    ...
    class ViewHold2(itemView: View, listener: SurveyApprovalListListener) : ViewHold(itemView) {
...
//            }
        ...
        when (holder) {
            is ViewHold1 -> {
                ...
                holder.binding.update.setOnClickListener {
                    val tempSurveyApprovalData=SAcqSiteApproval()
                    ...
                    listener.updateSurveyApprovalClicked(tempSurveyApprovalData)
    ...
    interface SurveyApprovalListListener {
       fun attachmentItemClicked(item : Attachments)
       fun addAttachment()
       fun updateSurveyApprovalClicked(data:SAcqSiteApproval)
    }
