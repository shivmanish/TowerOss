...
import com.smarthub.baseapplication.model.siteIBoard.newOpcoTenency.RadioAntennaAndRRUCableDetail
...
class CableDetailsAdapter(var listener: CableDetailsItemClickListener, cableDetails: ArrayList<RadioAntennaAndRRUCableDetail>?, var context: Context) : RecyclerView.Adapter<CableDetailsAdapter.ViewHold>() {

    var list : ArrayList<RadioAntennaAndRRUCableDetail> = cableDetails!!
    var currentOpened = -1

    fun updateItem(pos : Int,data : RadioAntennaAndRRUCableDetail){
        list[pos] = data
    ...
    class ViewHold1(itemView: View,listener: CableDetailsItemClickListener) : ViewHold(itemView) {
...
//            }
    ...
    override fun onBindViewHolder(holder: ViewHold, position: Int) {
        if (holder is ViewHold1) {
            var data: RadioAntennaAndRRUCableDetail=list[position]
...
//                listener.editModeCliked(data,position)
            ...
            try {
                ...
                holder.binding.itemTitleStr.text = String.format(context.resources.getString(R.string.rf_antenna_title_str_formate),data.SerialNumber,AppPreferences.getInstance().getDropDownValue(DropDowns.CableName.name,data.CableName.get(0).toString()),data.CableType)
    ...
    interface CableDetailsItemClickListener{
