...
class PoleMaintenanceEditDialouge (var data: PreventiveMaintenance, var childId: Int?, var parentId: Int?, var listner:PoleMaintenanceUpdateListener) : BaseBottomSheetDialogFragment(){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewmodel.updateTwrCivilInfraDataResponse?.observe(viewLifecycleOwner) {
            if (it != null && it.status == Resource.Status.LOADING) {
                showProgressLayout()
                AppLogger.log("PoleMaintenanceEditDialouge data Updating in progress ")
                return@observe
            }
            if (it?.data != null && it.status == Resource.Status.SUCCESS) {
                AppLogger.log("PoleMaintenanceEditDialouge card Data Updated successfully")
                listner.updatedData()
                hideProgressLayout()
                dismiss()
...
//                Toast.makeText(context,"Something went wrong in update data . Try again", Toast.LENGTH_SHORT).show()
                ...
                AppLogger.log("PoleMaintenanceEditDialouge error :${it.message}, data : ${it.data}")
            } else {
                AppLogger.log("PoleMaintenanceEditDialouge Something went wrong in updating data")
    ...
    interface PoleMaintenanceUpdateListener{
        fun updatedData()
    }
